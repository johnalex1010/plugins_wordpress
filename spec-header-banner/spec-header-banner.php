...
function shb_get_asset_version($path)
{
  ...
  return file_exists($file) ? (string) filemtime($file) : '4.3';
}
...
add_filter('manage_shb_banner_posts_columns', function ($columns) {
  ...
  foreach ($columns as $key => $label) {
    ...
    if ($key === 'title') {
      $custom_columns['shb_publication_status'] = __('Estado', 'spec-header-banner');
      $custom_columns['shb_target_pages'] = __('Páginas del banner', 'spec-header-banner');
    }
  }
...
add_action('manage_shb_banner_posts_custom_column', function ($column, $post_id) {
  if ($column === 'shb_publication_status') {
    ...
    $status_label = $is_published ? __('Publicado', 'spec-header-banner') : __('No publicado', 'spec-header-banner');
  ...
  if ($column === 'shb_target_pages') {
    ...
    if (!$page_ids) {
      echo '<span class="shb-empty-column">' . esc_html__('Sin páginas asignadas', 'spec-header-banner') . '</span>';
      return;
    }
    ...
    foreach ($page_ids as $page_id) {
      ...
      $page_title = $page_title ? $page_title : sprintf(__('Página #%d', 'spec-header-banner'), $page_id);
...
add_action('add_meta_boxes', function () {
  add_meta_box('shb_config', __('Configuración del Banner', 'spec-header-banner'), 'shb_render_metabox', 'shb_banner', 'normal', 'high');
});
...
?>
  <div class="shb-admin">
    <button type="button" class="button" id="shb_upload_image_button"><?php echo esc_html__('Seleccionar Imagen', 'spec-header-banner'); ?></button>
    <input type="hidden" id="shb_image_id" name="shb_image_id" value="<?php echo esc_attr($image_id); ?>">
    <p class="description"><?php echo esc_html__('La imagen es obligatoria para publicar el banner.', 'spec-header-banner'); ?></p>
    <div id="shb_required_notice" class="notice notice-error shb-required-notice">
      <p><?php echo esc_html__('Selecciona una imagen antes de guardar el banner.', 'spec-header-banner'); ?></p>
    </div>
    ...
    <p class="shb-field">
      <label for="shb_link"><strong><?php echo esc_html__('Enlace del banner', 'spec-header-banner'); ?></strong></label>
      <input type="text" id="shb_link" name="shb_link" class="shb-field__control" value="<?php echo esc_attr($link); ?>" placeholder="https://... o #seccion">
      <span class="description"><?php echo esc_html__('Acepta URLs completas o anclas internas como #formulario_inscripcion.', 'spec-header-banner'); ?></span>
    </p>

    <p class="shb-field">
      <label for="shb_target"><strong><?php echo esc_html__('Target del enlace', 'spec-header-banner'); ?></strong></label>
      <select name="shb_target" id="shb_target" class="shb-field__select">
        <option value="_self" <?php selected($target, '_self'); ?>><?php echo esc_html__('Misma ventana (_self)', 'spec-header-banner'); ?></option>
        <option value="_blank" <?php selected($target, '_blank'); ?>><?php echo esc_html__('Nueva ventana (_blank)', 'spec-header-banner'); ?></option>
      </select>
    </p>

    <h4><?php echo esc_html__('Páginas', 'spec-header-banner'); ?></h4>
    <div class="shb-page-selector">
      <label for="shb_page_search" class="screen-reader-text"><?php echo esc_html__('Buscar páginas', 'spec-header-banner'); ?></label>
      <input type="search" id="shb_page_search" class="shb-field__control shb-page-selector__search" placeholder="<?php echo esc_attr__('Buscar páginas...', 'spec-header-banner'); ?>">

      <div class="shb-page-selector__list" role="group" aria-label="<?php echo esc_attr__('Páginas disponibles', 'spec-header-banner'); ?>">
      ...
      <p class="description shb-page-selector__empty"><?php echo esc_html__('No se encontraron páginas con ese criterio.', 'spec-header-banner'); ?></p>
    </div>

    <h4><?php echo esc_html__('Páginas donde está activo un banner', 'spec-header-banner'); ?></h4>
    <table class="widefat striped shb-active-table">
      <thead>
        <tr>
          <th scope="col"><?php echo esc_html__('Banner', 'spec-header-banner'); ?></th>
          <th scope="col"><?php echo esc_html__('Páginas', 'spec-header-banner'); ?></th>
        </tr>
      </thead>
      <tbody>
        <?php if (!$active_items) : ?>
          <tr>
            <td colspan="2"><?php echo esc_html__('No hay banners activos con páginas asignadas.', 'spec-header-banner'); ?></td>
          </tr>
        <?php else : ?>
          <?php foreach ($active_items as $item) : ?>
            <?php
            $banner_title = $item['title'] ? $item['title'] : sprintf(__('Banner #%d', 'spec-header-banner'), $item['id']);
            $page_titles = [];

            foreach ($item['pages'] as $page_id) {
              $page_title = get_the_title($page_id);
              $page_titles[] = $page_title ? $page_title : sprintf(__('Página #%d', 'spec-header-banner'), $page_id);
            }
            ?>
            <tr>
...
?>
  <div class="notice notice-error is-dismissible">
    <p><?php echo esc_html__('El banner quedó como borrador porque la imagen es obligatoria para publicarlo.', 'spec-header-banner'); ?></p>
  </div>
<?php
});
